...
% "L6_5"
% };
% c = 8; %conditions in range 
% range = [3017,1722180]; %[firststim, laststim]

%PST
%conditions = {"CGL1", "CGL2", "CGL3", "CGL4"};
conditions = {"CGR1", "CGR2", "CGR3", "CGR4"};
c = 4; %conditions in range 
range = [76814,936911]; %[firststim, laststim]
...
n=20; %length of stimulation pulse (number of adjacent peaks to reference) k= 20 default
k=1; %strictness of peak distances to reference and compare; k=1 default;

%'MinPeakDistance' 
mpd = 475;  %Sample_rate/stim-1; 
%'MinPeakHeight' 
mph = 1 ;
%'MinPeakProminence' 
mpp = 100;
...
%Exclude Stimulation Range
exclude_ranges = []; %[first1 end1; first2 end2...]
...
filename = [filename, 'mff']; %filename = [filename, 'mff1'];
...
%Calculate which loc is a condition
num_samples = range(2)-range(1)+1;
%range(2) = endpoint;
endpoint=range(2);
starting_sample_condition = [range(1):ceil(num_samples/c):range(2)]; %starting point
...
for k = 1:height(L)
    %out of range
    if L.locs(k)>endpoint
        break
    end
    %if location is at the end of condition range, assign to next variable
    %and increase count
    if count<c && L.locs(k)>starting_sample_condition(count+1)
        count = count +1;
    end
    %assign condition
    Cond_Nums(k) = count; 
